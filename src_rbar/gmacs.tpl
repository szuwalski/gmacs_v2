...
// ================================================================================================
 ...
 END_CALCS
  ...
  init_vector Penalty_emphasis(1,9);                       ///> Weights on penalties
...
FUNCTION dvar_vector project_biomass(const int YrRef, const int iproj)
  ...
  for ( int i = 1982; i <= 2012; i++ ) { MMARef += MMAState(i); MLARef += MLAState(i); NF += 1; }
  MMARef /= NF; MLARef /= NF;
...
FUNCTION dvar_vector calc_predicted_project(const int YrRef, dvar3_array d4_Npass, dvar3_array _ft, dvar4_array _Z1, dvar4_array _Z2)
 {
  ...
  for (int isim=1;isim<=prj_replicates;isim++)
   {
    ...
    for (int irun=1;irun<=prj_Nstrat;irun++)
     ...
     // cout<<"Projectbio"<<endl;
	  // Why is this project_biomass instead of project_biomass_OFL?
